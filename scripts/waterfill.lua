...
local function edit(e)
    ...
    if not (e.item == 'rm-waterfill') then
        return
    end
    ...
    if #entities > 0 then
        flying_error(player, { 'waterfill.entities' })
        return
    end
    ...
    if t == 0 and too_far == 0 then
        flying_error(player, { 'waterfill.not_here' })
    elseif t == 0 and too_far > 0 then
        flying_error(player, { 'waterfill.cannot_reach' })
    else
