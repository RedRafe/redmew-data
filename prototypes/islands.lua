-- Islands map generation
-- Code taken from FreightForwarding mod by Xorimuth taken from IslandStart mod by Yehn, which was itself taken from vanilla
...
local standard_starting_lake_elevation_expression = noise.define_noise_function(function(x, y, tile, map)
    ...
    local minimal_starting_lake_bottom = starting_lake_distance / 4 - minimal_starting_lake_depth + make_basis_noise_function(map.seed, 123, 1.5, 1 / 8)(x, y)
...
local function is_water_level_correct(to_be_corrected, map)
    return noise.ident(noise.max(map.wlc_elevation_minimum, to_be_corrected + map.wlc_elevation_offset * is_wlc_mult))
end
